...
const checkout = async (
    ...
    const { modules: cartModules, dependencies: cartDependencies } =
        await waitForBrowserCollector(page);

    await page.goto(`${baseUrl}checkout`, { waitUntil: 'networkidle0' });
    const { modules: checkoutModules, dependencies: checkoutDependencies } =
        await waitForBrowserCollector(page);

    merge(
        bundleConfig.modules,
        orderModules(cartModules, cartDependencies),
        orderModules(checkoutModules, checkoutDependencies)
    );
