...
module.exports = async (page) => {
    ...
    const moduleLoadOrder = new Map();
    let level = 0;
    // Transfer 'dependencyMap' to 'moduleLoadOrder'
    while (dependencyMap.size) {
        if (level > 999) {
            // Something went wrong. Fall back to un-ordered module list.
            return modules;
        }
        ...
        for (const [moduleName, dependencies] of dependencyMap) {
            ...
            moduleLoadOrder.set(moduleName, level);
            modulesAtThisLevel.push(moduleName);
            dependencyMap.delete(moduleName);
        }

        // No more empty elements left; we descend another level
        level++;
        ...
        for (const foundModuleName of modulesAtThisLevel) {
            for (const dependencies of dependencyMap.values()) {
                delete dependencies[foundModuleName];
            }
        }
    }
    ...
    // Transfer 'moduleLoadOrder' to 'orderedModules'
    for (level = 0; level <= maxLevel; level++) {
        const modulesAtThisLevel = [];
        for (const [moduleName, moduleLevel] of moduleLoadOrder) {
            if (moduleLevel == level) {
                modulesAtThisLevel.push(moduleName);
                moduleLoadOrder.delete(moduleName);
            }
        }
