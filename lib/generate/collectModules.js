...
module.exports = async (page) => {
     ...
     */
    console.log('Wait to make sure RequireJS is loaded.');
     ...
     */
    console.log('Use Magento\'s rjsResolver to wait for all modules to load.');
    await page.evaluate(() => {
        // Create a callback function to handle rjsResolver completion
        function onRjsResolverComplete() {
            console.log('rjsResolver completed');
	}
        require(['rjsResolver'], (resolver) => {
            console.log('rjsResolver started');
            resolver(onRjsResolverComplete);
        });
    });
...
//    });
     ...
     */
    console.log('Wait for browser to be idle for a good measure.');
    await page.evaluate(
        () =>
            new Promise((resolve) => {
     ...
     */
    console.log('Wait another 5s for a good measure.');
    ...
    const modules = await page.evaluate((excludedModules) => {
         ...
         */
        console.log('Prepare a separate context where modules are not assigned to bundles.');
        console.log('This make it possible to fetch real module paths even with bundling enabled.');
        ...
        Object.keys(window.require.s.contexts._.defined).forEach(
            (moduleName) => {
                 ...
                 */
		console.log('Ignore all modules that are loaded with plugins other than text.');
                if (
                    (moduleName.includes('!') &&
